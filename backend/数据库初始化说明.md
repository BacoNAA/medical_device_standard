...
SQL脚本位于 `backend/src/main/resources/sql/` 目录下：

| 脚本文件 | 说明 | 执行顺序 |
|---------|------|---------|
| `01_create_database.sql` | 创建数据库 `medical_device_standards` | 1 |
| `02_create_tables.sql` | 创建15个数据表（PascalCase命名） | 2 |
| `03_insert_init_data.sql` | 插入初始数据（6个测试账号） | 3 |

---

## 执行方式

使用VSCode MySQL扩展按顺序执行：
...
详细步骤参考：[VSCode数据库连接指南.md](./VSCode数据库连接指南.md)
...
- **VSCode连接指南：** [VSCode数据库连接指南.md](./VSCode数据库连接指南.md)
- **连接验证指南：** [数据库连接验证指南.md](./数据库连接验证指南.md)
- **项目结构说明：** [项目结构说明.md](./项目结构说明.md)
- **数据库设计文档：** `project_document/数据库设计文档-医疗器械标准数据库与应用系统_v2.0.md`
