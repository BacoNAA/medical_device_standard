...
TRACKING_DATE = datetime(2025, 11, 4)
TRACKING_LINE_COLOR = 'FF0000'
COMPLETED_FILL_COLOR = '00B050'
OUTPUT_FILENAME = f"项目甘特图_{TRACKING_DATE.strftime('%m%d')}跟踪.xlsx"
...
def create_gantt_chart():
    ...
    while current_date <= project_end:
        ...
        header_value = f"W{week_num}\n{date_str}\n{weekday}"
        if current_date.date() == tracking_date.date():
            header_value += "\n跟踪"
    ...
    tracking_key = tracking_date.strftime('%Y-%m-%d')
    tracking_col = date_columns.get(tracking_key)
    if tracking_col is None:
        raise ValueError(f"Tracking date {tracking_key} is outside the project range.")
    ...
    tracking_line_side = Side(style='medium', color=TRACKING_LINE_COLOR)
    ...
    for task in tasks:
        ...
        while current <= end_date:
            ...
            col_idx = date_columns.get(date_key)
            if col_idx is not None:
                cell = ws.cell(row=row, column=col_idx)
                fill_color = COMPLETED_FILL_COLOR if current <= tracking_date else bar_color
                cell.fill = PatternFill(start_color=fill_color, end_color=fill_color, fill_type='solid')
    ...
    def apply_tracking_border(cell):
        existing_border = cell.border
        top_side = existing_border.top if (existing_border and existing_border.top and existing_border.top.style) else thin_border.top
        bottom_side = existing_border.bottom if (existing_border and existing_border.bottom and existing_border.bottom.style) else thin_border.bottom
        cell.border = Border(
            left=tracking_line_side,
            right=tracking_line_side,
            top=top_side,
            bottom=bottom_side
        )

    for r in range(1, last_task_row + 1):
        cell = ws.cell(row=r, column=tracking_col)
        apply_tracking_border(cell)
    ...
    # 进度追踪说明
    legend_row += 2
    ws.cell(row=legend_row, column=1, value='进度追踪说明：').font = Font(bold=True, size=10)
    legend_row += 1

    completed_cell = ws.cell(row=legend_row, column=1)
    completed_cell.fill = PatternFill(start_color=COMPLETED_FILL_COLOR, end_color=COMPLETED_FILL_COLOR, fill_type='solid')
    completed_cell.border = thin_border
    ws.cell(row=legend_row, column=2, value='绿色：追踪日前已完成的计划进度').font = Font(size=9)

    legend_row += 1
    tracking_cell = ws.cell(row=legend_row, column=1)
    tracking_cell.border = Border(
        left=tracking_line_side,
        right=tracking_line_side,
        top=Side(style='thin', color='000000'),
        bottom=Side(style='thin', color='000000')
    )
    ws.cell(row=legend_row, column=2, value=f'红线：{tracking_date.strftime("%Y-%m-%d")} 跟踪日').font = Font(size=9)
    ...
    print(f'[OK] 跟踪日期：{tracking_date.strftime("%Y-%m-%d")}')
